                        ...
                        <v-col cols="12" md="1">
                            ...
                            >
                                <svg
                                    ...
                                    height="28"
                                    fill="rgb(29, 111, 66)"
                                    class="bi bi-file-earmark-excel mt-1"
                                    viewBox="0 0 16 16"
                                >
                                    <path
                            ...
                            <template v-if="hasMedia(r.num_media)">
                                ...
                                <template v-else>
                                    <v-btn
                                        variant="text"
                                        icon="mdi-attachment"
                                        @click="refreshMediaList(r.sid)"
                                    ></v-btn>
                                </template>
                            </template>
                        </td>
                        <td :title="r.date_updated">
                            {{ localDate(r.date_updated) }}
                        </td>
...
useSeoMeta({
    title: "Twilio Message History",
    description:
        "Your Twilio account's message history in an easy to view format.",
});
...
async function loadData(refreshCache: boolean, { appendResults = false } = {}) {
    ...
    if (error) {
        ...
        if (appendResults) {
            (result.messages as Array<any>).forEach((x) =>
...
const xlsxFileName = () => {
    ...
    if (state.value.form.from_date) {
        filename_parts.push(
            dayjs(state.value.form.from_date).format("YYYYMMDD")
        );
    }
